  ...
  module Keyword
    module Synth
      class Synthesizer < Narra::SPI::Synthesizer
        ...
        @options = {
            meta_fields_to_analyze: 'keywords'
        }
        ...
        def synthesize(options = {})
          ...
          fields = Synthesizer.to_array(Synthesizer.parse_options(:meta_fields_to_analyze, options))
          ...
          @project.items.each do |item1|
            @project.items.each do |item2|
              ...
              add_junction([item1, item2], weight: weight, synthesizer: Synthesizer.identifier) if weight > 0
            end
